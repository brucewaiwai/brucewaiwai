...
void turn_on_robot::Publish_Voltage()
{
    ...
    std_msgs::Bool em_button_msgs;//定义电源电压发布topic的数据类型std_msgs::Float32
    ...
    if(Count_Voltage_Pub++>10)
      {
        ...
        em_button_msgs.data = EM_button;//em_button
        em_button_punlisher.publish(em_button_msgs);//em_button
      }
}
/**************************************
Date: June 29, 2020
 ...
 if (Receive_Data.Frame_Header == FRAME_HEADER )//判断帧头
  {
    ...
    { 
      ...
      { 
        ...
        EM_button = 1 - Receive_Data.rx[21]; 
        return true;
     }
    }
  } 
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  em_button_punlisher = n.advertise<std_msgs::Bool>("/EM_button", 10);//电池电压数据发布
