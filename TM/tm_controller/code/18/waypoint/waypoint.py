...
class waypoint:
    ...
    def __init__(self):
        ...
        self.sub_local_plan = rospy.Subscriber('/move_base/DWAPlannerROS/local_plan',Path,self.local_plan_callback)
        ...
        self.local_plan = True
        self.local_plan_count = 0

        self.move_base_data = 0
        self.len_local_plan = 0
        self.len_global_plan = 0

    def local_plan_callback(self,data):
        self.len_local_plan = len(data.poses)
        # print(self.len_local_plan)
        if self.len_local_plan <= 20:
            self.local_plan_count += 1
        else:
            self.local_plan_count = 0

        if self.local_plan_count >= 10:
                ...
                while True:
                    ...
                    if dis_to_goal <= self.next_point_dis:
                        ...
                        if point_cmd == 'stop':
                            ...
                            ################################################################
                            if self.global_plan == False:
                                # self.global_plan = 1
                                
                                print('no plan...')
                                self.state = 'no_plan'
                                
                            elif self.global_plan == True:
                                if self.local_plan == False:
                                    self.state = 'no_plan'
                                else:
                                    self.state = 'running'
                                    self.skiped_pt = 0
                    ...
                    print(cmd)
                    print('point',self.point, pt_id, point_cmd)
                    print('skip_pt',self.skiped_pt)
                    
                    print('global plan len',self.len_global_plan)
                    print('global_plan_count',self.global_plan_count)
                    print('local plan len',self.len_local_plan)
                    print('local_plan_count',self.local_plan_count)
